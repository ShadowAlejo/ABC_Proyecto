...
Optimizaciones aplicadas:
  - Extracción y normalización de cabeza sin YuNet.
...
def extract_worker(task_info: tuple[int, Path, int, Path]):
    ...
    norm_face = normalize_face(img_bgr, is_body_roi=False)
    if norm_face is None:
        return start_idx, False

    q_report = validate_face_quality(norm_face, training_mode=True)
    ...
    # Escribir la imagen real (índice start_idx)
    ...
    for i in range(AUG_COUNT):
        ...
        aug_face = augmenter.apply_random_erasing(norm_face.copy(), sl=0.02, sh=0.15, r1=0.3, r2=3.3)
...
def main():
    logger.info("Iniciando preparación Out-of-Core de dataset sin YuNet...")
    ...
    logger.info(f"F1-Macro CV Global: {mean_f1:.4f}")
    ...
    if mean_f1 >= 0.70:
        ...
        logger.warning(f"[ALERTA] El F1-Macro CV ({mean_f1:.4f}) es menor al umbral. NO se guardará.")
